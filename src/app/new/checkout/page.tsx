...
export default function CheckoutPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      saveCart([]);
      router.push(`/new/checkout/complete?orderId=${orderId}`);
    } catch (err) {
      console.error(err);
  ...
  return (
    <main className="min-h-screen bg-gray-50">
      <header className="bg-white border-b border-gray-200 px-6 py-4 flex items-center gap-4">
        <Link href="/new" className="text-gray-400 hover:text-gray-700 transition-colors">
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
          </svg>
        </Link>
        <h1 className="text-lg font-bold text-gray-900">주문서</h1>
      </header>
      ...
      <form onSubmit={handleSubmit} className="max-w-2xl mx-auto px-6 py-8 space-y-6">
        ...
        <section className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          ...
          {cart.length === 0 ? (
            <div className="px-5 py-8 text-center text-gray-400 text-sm">
              장바구니가 비어 있습니다.{" "}
              <Link href="/new" className="underline text-gray-600">상품 보러 가기</Link>
            </div>
          ) : (
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1.5">
                  주문자 성함 <span className="text-red-400">*</span>
                </label>
                <input
                  type="text"
                  required
        ...
        <button
          ...
          className="w-full bg-gray-900 hover:bg-gray-700 disabled:bg-gray-300 text-white font-semibold py-4 rounded-xl transition-colors text-sm"
        >
          {submitting ? "주문 처리 중..." : "주문하기"}
        </button>

        <p className="text-center text-xs text-gray-400">
          주문 후 담당자 확인을 통해 최종 접수됩니다.
        </p>
      </form>
    </main>
  );
