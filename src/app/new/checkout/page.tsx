...
import { CartItem } from "@/app/new/page";
...
export default function CheckoutPage() {
  ...
  async function handleSubmit(e: { preventDefault(): void }) {
    ...
    try {
      for (const c of cart) {
        await createOrder({
          userId: user.uid, userEmail: user.email ?? "", type: "retail",
          items: [{ productId: c.productId, productName: c.productName, category: c.category, yongyang: c.yongyang, quantity: c.quantity }],
          ordererName: form.ordererName, phone: form.phone, address: fullAddress, deliveryDate: form.deliveryDate, memo: form.memo,
          groupId,
        });
      }
      saveCart([]);
      router.push(`/new/checkout/complete?groupId=${groupId}`);
    } catch (err) {
      console.error(err);
  ...
  return (
    <main className="min-h-screen">
      <header className="bg-white/80 backdrop-blur-sm border-b border-stone-200/60 px-6 py-4 flex items-center gap-4">
        <Link href="/new" className="text-stone-400 hover:text-stone-700 transition-colors">
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
          </svg>
        </Link>
        <Logo />
        <span className="ml-2 text-sm font-semibold text-stone-700">소매 주문서</span>
      </header>
      ...
      <form onSubmit={handleSubmit} className="max-w-2xl mx-auto px-6 py-8 space-y-5">
        <section className="bg-white rounded-2xl border border-stone-200 overflow-hidden">
          ...
          {cart.length === 0 ? (
            <div className="px-5 py-10 text-center text-stone-400 text-sm">
              장바구니가 비어 있습니다.{" "}
              <Link href="/new" className="text-orange-600 hover:underline font-medium">상품 보러 가기</Link>
            </div>
          ) : (
        ...
        <p className="text-center text-xs text-stone-400">주문 후 담당자 확인을 통해 최종 접수됩니다.</p>
      </form>
    </main>
  );
}
